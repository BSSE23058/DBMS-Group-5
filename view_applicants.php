...
<head>
    ...
    <style>
        ...
        .applicants-section { padding: 5rem 0; background-color: #f8f9fa; }
        .applicant-card {
            border-radius: 15px;
...
<section class="applicants-section">
    <div class="container">
                    ...
                    <div class="col-md-6 col-lg-4">
                        <div class="card applicant-card">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                ...
                                <span class="badge status-<?php echo strtolower(htmlspecialchars($applicant['status'])); ?>">
                                    <?php echo htmlspecialchars($applicant['status']); ?>
                                </span>
                            </div>
                            <div class="card-body">
                                <p class="applicant-info"><i class="bi bi-envelope"></i> Email: <?php echo htmlspecialchars($applicant['applicant_email']); ?></p>
                                <p class="applicant-info"><i class="bi bi-calendar"></i> Applied On: <?php echo htmlspecialchars(date('M d, Y', strtotime($applicant['application_date']))); ?></p>
                                <?php if (!empty($applicant['resume_filepath'])): ?>
                                    <p class="applicant-info"><i class="bi bi-file-earmark-person"></i> <a href="<?php echo htmlspecialchars($applicant['resume_filepath']); ?>" target="_blank">View Resume</a></p>
                                <?php endif; ?>
                                <?php if (!empty($applicant['cover_letter'])): ?>
                                    <p class="applicant-info"><strong>Cover Letter:</strong></p>
                                    <p class="card-text small"><?php echo nl2br(htmlspecialchars($applicant['cover_letter'])); ?></p>
                                <?php endif; ?>
                                <hr>
                                <div class="d-flex justify-content-between flex-wrap mb-3">
                                    <!-- Status Update Forms -->
                                    <form action="update_applicant_status.php" method="POST" style="display:inline;" class="mb-2">
                                    ...
                                    <a href="mailto:<?php echo htmlspecialchars($applicant['applicant_email']); ?>?subject=Regarding your application for <?php echo urlencode($job_row['title']); ?>&body=Dear <?php echo htmlspecialchars($applicant['applicant_name']); ?>," class="btn btn-message btn-applicant-action mt-2 w-100">
                                ...
                                <div class="schedule-form-section">
                                    ...
                                    <?php if ($applicant['interview_date'] && $applicant['interview_time']): ?>
                                        <p class="mb-1">
                                            ...
                                            <span class="fw-bold"><?php echo htmlspecialchars(date('M d, Y', strtotime($applicant['interview_date']))); ?></span> at
                                            <span class="fw-bold"><?php echo htmlspecialchars(date('h:i A', strtotime($applicant['interview_time']))); ?></span>
                                        </p>
                                    ...
                                    <form action="schedule_interview.php" method="POST">
                                        ...
                                        <div class="row g-2 mb-2">
                                            <div class="col-md-6">
                                                <input type="date" class="form-control form-control-sm" name="interview_date"
                                                       value="<?php echo htmlspecialchars($applicant['interview_date'] ?: ''); ?>" required>
                                            </div>
                                            <div class="col-md-6">
                                                <input type="time" class="form-control form-control-sm" name="interview_time"
                                                       value="<?php echo htmlspecialchars($applicant['interview_time'] ?: ''); ?>" required>
                                            </div>
                                        </div>
        ...
        <div class="text-center mt-4">
            <a href="job_listings.php" class="btn btn-secondary"><i class="bi bi-arrow-left"></i> Back to All Jobs</a>
        </div>
        </div>
    </div>
</section>
