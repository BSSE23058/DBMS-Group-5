...
if (isset($_SESSION['user_id'])) {
    ...
    } elseif ($_SESSION['user_type'] === 'job_seeker') {
...
if (isset($_SESSION['user_id']) && $_SESSION['user_type'] === 'employer' && isset($_GET['view']) && $_GET['view'] === 'my_posts') {
...
<head>
    ...
    <style>
        ...
        .job-card { /* Original class for the smaller cards */
            border-radius: 15px;
        ...
        .promoted-badge { /* This style was in the previous code, keeping it */
...
<div class="container mt-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <?php if ($dashboard_link): // Only show if a user is logged in and has a dashboard ?>
            <a href="<?php echo htmlspecialchars($dashboard_link); ?>" class="btn btn-primary">
                    ...
                    <div class="card job-card"> <!-- Restored card and job-card classes -->
                        ...
                        <div class="card-body">
                            ...
                            <p class="meta-info mb-1">
                                <span><i class="bi bi-geo-alt"></i> <?php echo htmlspecialchars($job['location'] ?: 'N/A'); ?> (<?php echo htmlspecialchars($job['work_mode'] ?: 'N/A'); ?>)</span>
                            </p>
                            <p class="meta-info mb-3">
                                <span><i class="bi bi-briefcase"></i> <?php echo htmlspecialchars($job['job_type']); ?></span>
                                <?php if (!empty($job['category_name'])): ?>
                                    <span class="badge bg-secondary ms-2"><?php echo htmlspecialchars($job['category_name']); ?></span>
                                <?php endif; ?>
                            </p>
                            <p class="salary-info mb-3">
                                <i class="bi bi-currency-dollar"></i> Salary: <?php echo htmlspecialchars($job['salary'] ?: 'Negotiable'); ?>
                            </p>
                            <p class="card-text description-text"><?php echo nl2br(htmlspecialchars($job['description'])); ?></p>
                            ...
                            <div class="d-flex justify-content-start gap-2 mt-4 job-card-actions">
                                ...
                                ?>
                                    <a href="apply_job.php?job_id=<?php echo htmlspecialchars($job['id']); ?>" class="btn btn-apply flex-grow-1 me-2">Apply Now</a>
                                <?php endif; ?>

                                <?php
                                // Conditional display for employer action buttons
                                // Show ONLY if an employer is logged in AND it's THEIR job
                                if (isset($_SESSION['user_type']) && $_SESSION['user_type'] === 'employer' && isset($_SESSION['user_id']) && $_SESSION['user_id'] == $job['employer_id']):
                                ?>
                                    <a href="view_applicants.php?job_id=<?php echo htmlspecialchars($job['id']); ?>" class="btn btn-outline-secondary">View Applicants</a>
                                    <a href="edit_job.php?id=<?php echo htmlspecialchars($job['id']); ?>" class="btn btn-outline-secondary">Edit Job</a>
                                    <a href="delete_job.php?id=<?php echo htmlspecialchars($job['id']); ?>" class="btn btn-outline-danger" onclick="return confirm('Are you sure you want to delete this job posting?');">Delete Job</a>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
        ...
        <div class="row">
            ...
            <div class="col-lg-3 col-md-4 mb-4">
                ...
                <ul class="list-unstyled">
                    ...
                    <li class="mb-2"><a href="#" class="text-white text-decoration-none">Terms and Condition</a></li>
                </ul>
            </div>
